"""
Gemini AI client for structured SDS/TDS extraction.
...
class GeminiClient:
    """Gemini AI client for SDS/TDS structured extraction."""
...
Extract only pictograms explicitly mentioned or shown in the SDS."""
    ...
    def extract_from_documents(
        ...
        prompt = self._build_prompt(sds_text, tds_text, product_name)
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                return self._parse_response(response.text, product_name)
                ...
                if attempt == self.max_retries:
                    ...
                    try:
                        return self._repair_and_parse(response.text, product_name)
                    except Exception as repair_error:
                        logger.error(f"JSON repair failed: {repair_error}")
                        raise ValueError(
                            f"Gemini returned invalid JSON after {self.max_retries} attempts. "
        ...
        raise ValueError("All Gemini extraction attempts failed")
    ...
    def _repair_and_parse(self, response_text: str, product_name: str) -> ExtractedData:
        ...
        logger.info("Attempting JSON repair with Gemini")
